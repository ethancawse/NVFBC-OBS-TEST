...
// This function should have no local C++ objects with nontrivial destructors.
...
bool pick_desktop_resolution_and_adapter(int &outWidth, int &outHeight, int &outAdapterIndex)
{
    ...
    while (EnumDisplayDevices(NULL, adapterIndex, &dd, 0)) {
        // Check if this display device is the primary one.
        if (dd.StateFlags & DISPLAY_DEVICE_PRIMARY_DEVICE) {
            // Use the device name from this display device.
            DEVMODE dm;
    ...
    if (!foundPrimary) {
        // If we don't find a primary display, we fall back to the first adapter.
        // You may want to adjust this fallback to suit your needs.
        outWidth = 1920;
        outHeight = 1080;
        outAdapterIndex = 0;
    }
...
static void *nvfbc_dx9_create(obs_data_t *settings, obs_source_t *source)
{
    ...
    ctx->frame_interval = 1.0f / 60.0f;
    ctx->last_time      = std::chrono::steady_clock::now();

    int adapterIndex = 0;
    int width = 0, height = 0;

    blog(LOG_INFO, "Picking desktop resolution", adapterIndex);
    ...
    obs_enter_graphics();
    ctx->obs_texture = gs_texture_create(width, height, GS_BGRA, 1, nullptr, GS_DYNAMIC); //GS_BGRA
    obs_leave_graphics();
...
static void nvfbc_dx9_video_tick(void *data, float seconds)
{
    ...
    // Rely on OBS's tick timing and call grab_frame every tick
...
static bool create_d3d9_device(nvfbc_dx9_data *ctx, int adapterIndex)
{
    ...
    d3dpp.Windowed             = TRUE;
    // For windowed mode, the backbuffer format can be set to D3DFMT_UNKNOWN.
    ...
    // Instead of creating an offscreen plain surface, create a render target.
    ...
    // Create a system-memory surface for GetRenderTargetData.
...
static bool init_nvfbc_dx9(nvfbc_dx9_data *ctx, int adapterIndex)
{
    ...
    // Allocate variables for maximum display dimensions
    ...
    setupParams.eMode          = NVFBC_TODX9VID_ARGB; //D3DFMT_X8R8G8B8
...
{
    if (!ctx->fbc_initialized) // HSDFSDFHDJSKFHSKDJHFJK
        return false;
